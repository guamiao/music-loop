...
> 本次更新仅改动应用代码（`.vue` / `.js`），用户端无需重新安装，刷新页面即可生效。

### 优化

- **区分"启动引擎中"阶段**：引擎下载到 100% 后，还需在手机本地实例化 WASM（通常几秒到几十秒），此前界面停在"准备中"+进度 100%，容易误以为卡死。现单独显示"启动引擎中…"（进度条变为不定进度动画）并提示保持前台、不要锁屏，随后才进入"提取中"
