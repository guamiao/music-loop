...
> 本次更新改动应用代码与 SW 配置，用户端无需重新安装，刷新页面即可生效。

### 新增

- **国内 CDN 加速引擎下载**：32MB 的 ffmpeg 引擎改为优先从 **npmmirror（阿里国内 CDN）** 下载，GitHub Pages 同源文件作为自动兜底源；任一源失败自动无缝切换。解决手机端跨国下载慢、易断流报"核心文件下载中断"的问题
- **打开应用即后台预热引擎**：应用加载后在浏览器空闲时静默下载并初始化引擎，用户选视频的时间即可消化掉 CDN 冷节点的首次回源延迟，点「提取」时通常已就绪
- 下载成功后以固定版本键写入 Cache Storage，下载一次永久秒开、可离线

### 优化

- 下载改为**整包流式 + 断点续传**：优先单一连接整包下载（冷节点只需回源一次），中途断流自动用 HTTP Range 从断点续传；首字节阶段给予冷节点更长宽限并提示"连接中"，开始接收后 30 秒无数据才判定断流，全程有实时进度，不再无反馈干等

## [1.0.9] - 2026-09-10

> 本次更新改动应用代码与下载策略，用户端无需重新安装，刷新页面即可生效。

### 修复

- **手机弱网下 32MB 引擎下载中断报"核心文件下载中断"**：
  - 改为 **HTTP Range 分块下载**（每块 2MB），单块失败只自动重试该块（最多 8 次、指数退避），网络波动不再导致整个文件重下
  - 修复此前 `no-store` 导致**每次提取都强制重新下载 32MB** 的问题：下载成功后写入 Cache Storage，之后永久秒开、可离线
  - 服务器不支持 Range 时自动退化整文件流式下载
- **引擎在本地启动时切后台/锁屏导致永久卡住**：启动看门狗改为只累计前台时间；若启动期间页面进入过后台，回到前台自动用全新 worker 重启一次（iOS 会暂停后台 JS，旧 worker 可能已半死），累计前台等待超 2 分钟才报超时
- **修复 iPhone 上"提取音频并存入音乐库"按钮视觉上堆叠成三个**：忙碌态与空闲态拆为两个独立节点（v-if/v-else）并强制独立合成层，规避 iOS WebKit 按钮状态切换后的重绘残影
- 下载/启动/转码三阶段提示更明确
